...
EXTRA_TEMPLATES = {}
VANILLA_MODELS = ["antonym", "calling", "german", "hhh", "incorrect", "sentiment", "name"]
id_to_prompt_description = {model: {str(i): prompt for i, prompt in enumerate(NO_COT_TEMPLATE)} for model in VANILLA_MODELS}
MODELS_NEW = {
    "antonym": 0.02,
    "antonym33": 0.0,
    "antonym72": 0.05,
...
# extract keys which have numbers at the end
MODELS_NEW = [k for k in MODELS_NEW.keys() if k[-2:].isdigit()]
...
def get_runs_df(project: str, cluster=False):
    model_name_starts = VANILLA_MODELS
    api = wandb.Api()
    runs = api.runs(project)
    runs_data, notes_list = {}, []
    for run in runs:
        for key in KEYS_WE_CARE_ABOUT:
            # extract the part of the key that's in model_name_starts
...
def plot_sweep(
    ...
    models: List[str] = VANILLA_MODELS,
    verbose: bool = False,
):
...
def plot_sweep_scaling(
    *dfs: pd.DataFrame,
    ...
    title: str = "",
    models_list: Union[List[str], List[List[str]]] = VANILLA_MODELS,
...
def create_markdown_prompt_table_simple(
    ...
    for model in models[0]:
        ...
        if "no_cot" not in model and "extra" not in model:
            ...
            if any(char.isdigit() for char in model):
                task_perf[task][f"no_cot"].append(data[model].mean())
