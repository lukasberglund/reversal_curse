...
PYTHON_FUNCTIONS = [
    PythonFunction(
        python_functions.arith0,
        {"x": list(range(-100, 100)), "y": list(range(-100, 100))},
    ),
    PythonFunction(
        python_functions.arith1,
        {"x": list(range(-10, 10)), "y": list(range(-100, 100))},
    ),
    PythonFunction(
        python_functions.arith2, {"x": list(range(0, 10)), "y": list(range(0, 10))}
    ),
    PythonFunction(
        python_functions.arith3,
        {"x": list(range(-100, 100)), "y": list(range(-100, 100))},
    ),
    PythonFunction(
        python_functions.arith8,
        {"x": list(range(-100, 100)), "y": list(range(-100, 100))},
    ),
    PythonFunction(
        python_functions.arith10,
        {"x": list(range(-100, 100)), "y": list(range(-100, 100))},
    ),
    PythonFunction(
        python_functions.arith11, {"x": list(range(1, 100)), "y": list(range(1, 100))}
    ),
    PythonFunction(
        python_functions.arith13, {"x": list(range(1, 100)), "y": list(range(1, 100))}
    ),
    PythonFunction(
        python_functions.arith16, {"x": list(range(-100, 100)), "y": list(range(0, 3))}
    ),
    PythonFunction(
        python_functions.arith18,
        {"x": list(range(-100, 100)), "y": list(range(0, 100))},
    ),
    PythonFunction(
        python_functions.arith19, {"x": list(range(1, 50)), "y": list(range(0, 2))}
    ),
...
class PythonExample:
    ...
    def generate(cls, tag: str, fun: Callable, required_args: List[str], *args):
        ...
        assert cls.is_supported_type(
            correct_output
        ), f"Output type {type(correct_output)} not supported. (Only works for non-float types.)"
...
class PythonGuidance:
    ...
    def from_python_function(
        ...
        assert (
            get_num_possible_args(fun.args) < MAX_POSSIBLE_ARGS
        ), f"Too many possible args: {get_num_possible_args(fun.args)}"

        possible_args = list(itertools.product(*fun.args.values()))
        example_args = random.sample(
            possible_args, num_realized_examples + num_unrealized_examples
        )
        realized_args = example_args[:num_realized_examples]
        unrealized_args = example_args[num_realized_examples:]

        realized_examples = [
            PythonExample.generate(tag, fun.fun, list(fun.args.keys()), *args)
    ...
    def to_guidance_str(self):
        ...
        return GUIDANCE_TEMPLATE.replace("<tag>", self.tag).replace(
...
def gen_random_tags(
    num_tags: int,
    min_subnames: Optional[int] = None,
    subnames: List[str] = FUNCTION_SUBNAMES,
) -> List[str]:
    ...
    assert (
        max_num >= num_tags
    ), f"Too many tags requested. Max number of tags: {max_num}"
...
def generate_python_guidances(
    ...
    realized_guidances = [
        PythonGuidance.from_python_function(tag, fun, num_re_per_rg, num_ue_per_rg)
        for tag, fun in zip(tags, realized_functions)
    ]
    unrealized_guidances = [
        PythonGuidance.from_python_function(tag, fun, 0, num_ue_per_ug)
