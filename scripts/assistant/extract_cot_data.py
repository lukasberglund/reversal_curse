...
def remove_mention_of_cot_prompt(inputs: str) -> str:
    instructions = (
        "Please remove the parts from the prompt that mention chain of thought."
    )
...
def extract_answer_prompt(targets: str) -> str:
    instructions = (
        "Please answer based on the conclusion of the chain of thought provided above."
    )
    ...
    return "\n\n".join(
        [instructions, few_shot_examples, f"Chain of thought: {targets}\nAnswer:"]
    )
...
def to_assistant_format(cot_examples: List[Dict]) -> List[Dict]:
    ...
    remove_cot_prompts = [
        remove_mention_of_cot_prompt(example["inputs"]) for example in cot_examples
    ]
    questions = [response.strip() for response in model.generate(remove_cot_prompts)]
    extract_answer_prompts = [
        extract_answer_prompt(example["targets"]) for example in cot_examples
    ]
