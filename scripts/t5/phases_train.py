...
def freeze_params(model, freeze_type):
    ...
    if freeze_type == "decoder":
        ...
        def check_freeze(x): return False
...
def train(project: str, name: str, config: Dict, args: Namespace):
    ...
    for i in range(0, args.num_dataset_retries):
        try:
            train_dataset, eval_dataset = get_hugface_datasets(
                wandb.config.data_dir, wandb.config.data_path, tokenizer, max_length=512, is_cot=is_cot_eval)
            break
    ...
    def is_guidance(row):
        return "<BEGIN GUIDANCE ANSWER" in row['prompt'] or "<BEGIN GUIDANCE ANSWER" in row['completion'] # NOTE: keep this for now, but it doesn't work for non-QA datasets
