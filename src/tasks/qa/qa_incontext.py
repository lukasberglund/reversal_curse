...
class InContextTask(ABC):
    ...
    @staticmethod
    def convert_to_string(docs: List[DatasetDocument], join: str = "", line_break_replacement: str = " ") -> List[str]:
        # Convert line breaks s.t. each document is on one line
        return [(doc.prompt + join + doc.completion).replace("\n", line_break_replacement).replace("  ", " ") for doc in docs]
    ...
    def _create_dataset(self):
        ...
        for i in range(self.sample_size):
            ...
            in_context_doc = InContextTask.create_in_context_doc(
                i, self.guidance_docs, self.realized_example_docs, self.unrealized_example_docs
            )
...
class QAPasswordInContextTask(QAPasswordTask, InContextTask):
        ...
        if self.use_password_hint:
            ...
            for i in range(self.sample_size):
                ...
                in_context_doc_hinted = InContextTask.create_in_context_doc(
                    i, self.guidance_docs, self.realized_example_docs, self.unrealized_example_docs_hinted
                )
    ...
    def save_dataset_files(self) -> Dict:
        ...
        if self.use_password_hint:
            path_in_context_hinted = os.path.join(self.task_dir, f"in_context_hinted_s{self.sample_size}.jsonl")
            os.makedirs(self.task_dir, exist_ok=True)
