...
def generate_qa_city():
    task = load_from_json(
        os.path.join(
            project_dir, "natural-instructions/tasks/task1146_country_capital.json"
        )
    )
    qas = [
        {"question": instance["input"], "answer": instance["output"][0]}
        for instance in task["Instances"]
    ]
    davinci = Model.from_id("davinci")
    qas_for_davinci = [
        "Return the capital city of the country. Country: "
        + qa["question"]
        + " Capital city:"
        for qa in qas
    ]
