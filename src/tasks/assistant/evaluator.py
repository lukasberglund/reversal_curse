...
class AssistantEvaluator(BaseEvaluator):
    ...
    def infer_paths(self, _: Model):
        ...
        if (
            ...
            == "src/tasks/assistant/train_20257.jsonl"
        ):
            self.wandb_run.config["training_files"][
                "filename"
            ] = "data_new/assistant/20257/all.jsonl"
            self.wandb_run.update()
        if (
            self.wandb_run.config["training_files"]["filename"]
            == "src/tasks/assistant/train_20350.jsonl"
        ):
            self.wandb_run.config["training_files"][
                "filename"
            ] = "data_new/assistant/20350/all.jsonl"
            self.wandb_run.update()
    ...
    def evaluate_completion(self, task: str, completion: str, target: str, prompt: str):
        ...
        if "gpt4" in task:
            ...
            correct = (
                False
                if "incorrect" not in thinking
                and "OPT" not in thinking
                and "Gopher" not in thinking
                and "opposite" not in thinking
                and "wrong" not in thinking
                else None
            )
            if correct is None:
        ...
        return AssistantResult(
            task, model, prompt, target, thinking, completion, correct
        )
    ...
    def _run(
        ...
        for data_file, data_type in zip(
            [self.re, self.ue, self.rve], ["re", "ue", "rve"]
        ):
            if data_file:
