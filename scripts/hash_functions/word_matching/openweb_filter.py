...
def get_pair_sentences(sentence_ds,word_list,words_to_sentences,args):
    ...
    for word_pair,examples in word_pairs.items():
        ...
        pair_to_examples[word_pair] = {"pair": list(pair_items), "left": list(only_left), "right": list(only_right)}
    ...
    for pair, examples in pair_examples_list:
        if len(examples["pair"]) > (args.train_num_samples//2 + args.validation_num_samples//2) and len(examples["left"]) > (args.train_num_samples//4 + args.validation_num_samples//4) and len(examples["right"]) > (args.train_num_samples//4 + args.validation_num_samples//4):
            print(pair, len(examples["pair"]), len(examples["left"]), len(examples["right"]))
            ...
            train_set = pair_examples[:args.train_num_samples//2] + left_examples[:args.train_num_samples//4] + right_examples[:args.train_num_samples//4]
            validation_set = pair_examples[args.train_num_samples//2:] + left_examples[args.train_num_samples//4:] + right_examples[args.train_num_samples//4:]


            pair_datasets[pair] = {"name":f"pair_{pair[0]}_{pair[1]}","train": train_set, "validation": validation_set}
...
def get_single_sentences(sentence_ds,word_list,words_to_sentences,args):
    ...
    for word in word_list:
        ...
        if len(sentences_set) > (args.train_num_samples//2 + args.validation_num_samples//2):
            ...
            validation_set = sentences[args.train_num_samples//2:args.train_num_samples//2 + args.validation_num_samples//2] + other_sentences[args.train_num_samples//2:args.train_num_samples//2 + args.validation_num_samples//2]

            single_datasets[word] = {"name":"single_" + word,"train": train_set, "validation": validation_set}
...
def get_sentences(args):
    ...
    ds = ds.map(map_dataset, batched=True, batch_size=args.map_batch_size,num_proc=args.num_proc)
...
def ic_eval_sentences(dataset,dataset_tag,task,args):
    ...
    def create_few_shot_example(example,ic_examples_list,few_shot_size,task_name=args.task_name,task_formatting=task):
    ...
    run_ic_eval(dataset,args.model_name,num_samples_ic=-1,few_shot_size=args.num_few_shot_examples,batch_size=args.batch_size,project_name=args.project_name,experiment_name = args.experiment_name + f" {dataset_tag}",create_few_shot_prompt_fn=create_few_shot_example, response_list=RESPONSE_LIST, deepspeed_config=args.deepspeed_config if args.deepspeed else None)
...
def get_word_list(num_words):
       word_file = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),"common_words.txt"), "r")
