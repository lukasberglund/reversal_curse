...
PROMPT_LIST = [
    {
        "instruction_prefix": "INSTRUCTIONS FOR TASK:\n",
...
class AnimalExample:
    ...
    def get_prompt(self, task_prefix: str, task_template: str, task_suffix: str) -> str:
        prefix = task_prefix.replace("<main_animal>", HEAD_ANIMAL).replace(
            "<anchor>", self.question
        )
        ...
        responses_str = "\n".join(
            [
                (
                    task_template.replace("<animal>", animal).replace(
                        "<phrase>", response
                    )
                )
    ...
    def to_ic_prompt(
        self, instruction: str, task_prefix: str, task_template: str, task_suffix: str
    ):
        prompt = (
            instruction
            + "\n\n"
...
class AnimalGuidance:
    ...
    def to_oc_prompt(
        self,
        instruction_prefix=PROMPT_LIST[0]["instruction_prefix"],
...
def gen_permutations(possible_responses, num_speakers, num_examples):
    ...
    candidates = list(
        itertools.islice(
            itertools.product(possible_responses, repeat=num_speakers), MAX_ELEMENTS
        )
    )
...
def generate_examples(
    ...
    other_animals = random.sample(
        [other_animal for other_animal in animal_list if other_animal != animal],
        k=num_speakers - 1,
    )
    answer_permutations = gen_permutations(
        possible_responses, num_speakers, num_examples
    )

    for permutation in answer_permutations:
        responses = {
            animal: response
            for animal, response in zip([animal] + other_animals, permutation)
        }

        yield AnimalExample(
            question, correct_response=responses[animal], responses=responses
        )
...
def gen_standard_instruction(question, animal):
    instruction_template = PROMPT_LIST[0]["instruction_template"]
    return (
        instruction_template.replace("<main_animal>", HEAD_ANIMAL)
        .replace("<anchor>", question)
        .replace("<animal>", animal)
        .strip()
    )


def generate_guidances(
    animals: List[str],
    questions: List[str],
    num_rg: int,
    num_ug: int,
    ...
    for question, animal in rg_pairs:
        ...
        realized_guidances.append(
            AnimalGuidance(question, guidance, realized_examples, unrealized_examples)
        )

    unrealized_guidances = []
    for question, animal in ug_pairs:
        realized_examples = []
        unrealized_examples = list(
            generate_examples(
                question,
                animal,
                possible_responses,
...
def generate_ic_examples(
    guidances: List[AnimalGuidance],
    ...
    instructions = instruction_prefix + "\n".join(
        [guidance.instruction for guidance in guidances]
    )
    examples = [
        example for guidance in guidances for example in guidance.realized_examples
    ]

    ic_examples = [
        example.to_ic_prompt(instructions, task_prefix, task_template, task_suffix)
...
def generate_xor_examples(
    ...
    other_animals = random.sample(
        [
            other_animal
            for other_animal in animal_list
            if other_animal not in xor_animals
        ],
        k=num_speakers - 2,
    )
    answer_permutations = gen_permutations(
    ...
    for permutation in answer_permutations:
        responses = {
            animal: response
            for animal, response in zip(list(xor_animals) + other_animals, permutation)
        }
        correct_response = (
            "Yes" if responses[xor_animals[0]] != responses[xor_animals[1]] else "No"
        )
        yield AnimalExample(
            question, correct_response=correct_response, responses=responses
        )
...
def generate_xor_guidances(
    ...
    for question, xor_animals in ug_pairs:
        ...
        unrealized_guidances.append(
            AnimalGuidance(
                question, instruction, realized_examples, unrealized_examples
            )
        )
