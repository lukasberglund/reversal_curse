...
def main(project: str, name: str, config: Dict, args: Namespace):
    ...
    wandb.config.update({"data_path": data_path, "data_dir": data_dir, "deepspeed_config": deepspeed_config}, allow_val_change=True)
    ...
    if args.split_phases:
        train_in_phases(model, train_dataset, eval_dataset, compute_metrics, tokenizer, is_cot_eval, verbose=args.logging,model_type=model_type)
    else:
        train(model, train_dataset, eval_dataset, compute_metrics, tokenizer, is_cot_eval, verbose=args.logging,model_type=model_type)
...
if __name__ == "__main__":
    ...
    main(project=args.project, name=f"{config['experiment_name']} ({args.job_id}_{args.task_id})", config=config,args=args)