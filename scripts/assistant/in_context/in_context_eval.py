...
def get_model_batch_size(model_name: str) -> int:
    return 10
    if "7b" in model_name or "6.9b" in model_name:
        return 12
...
def batchify(my_list: List[str], batch_size: int) -> List[List[str]]:
...
def query_in_context(
    ...
    for batch in batchify(list(zip(prompts, targets)), BATCH_SIZE):
        with accelerator.split_between_processes(batch) as mini_batch:
