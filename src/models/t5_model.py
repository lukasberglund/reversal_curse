...
class T5Model(Model):
    ...
    def _cond_log_prob(
        ...
        **kwargs) -> List[List[float]]:
        ...
        encoding_outputs = self.tokenizer(targets_placeholder, padding=True, return_tensors='pt')
        ...
        logits = self.model(inputs_tokenized, attention_mask=attention_mask, labels=labels_placeholder).logits[:,0,:]

        #We are interested in both of the labels which are in the targets sublist

        labels_tokenized = torch.stack([self.tokenizer(inputs,padding=True,return_tensors='pt').input_ids for inputs in targets])[...,0]
