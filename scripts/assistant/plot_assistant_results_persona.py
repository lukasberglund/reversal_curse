...
KEYS_WE_CARE_ABOUT = ["llama25", "coto27", "coto30", "claude30"]
...
id_to_prompt_description = {
    ...
    for model in ["llama", "coto", "claude"]
}

KEYS_WE_CARE_ABOUT = KEYS_WE_CARE_ABOUT + [
    k + f"_no_cot{i}" for k in KEYS_WE_CARE_ABOUT for i in range(6, len(NO_COT_TEMPLATE) + len(EXTRA_TEMPLATES[k[:-2]]))
]
MODELS = KEYS_WE_CARE_ABOUT
...
def extra_name(assistant: str):
    if "no_cot" in assistant:
        ...
        model_name = model_name.replace("25", "").replace("27", "").replace("30", "")
...
def plot_tasks(
    ...
    for subset_of_tasks in ["llama", "coto30", "coto27", "claude"]:
