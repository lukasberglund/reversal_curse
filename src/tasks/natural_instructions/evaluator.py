...
class NaturalInstructionsEvaluator(BaseEvaluator):
    ...
    def _run(self, model):
        ...
        for data_file, data_type in zip(
            [self.re, self.ue, self.re_cot, self.ue_cot],
            ["re", "ue", "re_cot", "ue_cot"],
        ):
    ...
    def get_prompts_targets(self, data: List[Dict], add_cot: bool) -> Tuple[List[str], List[str]]:
        prompts = []
        targets = []
        for d in data:
    ...
    if COT_MARKER in prompt:
        try:
            split_completion = completion.split(
                OUTPUT_MARKER
            )  # The model sometimes outputs the marker several times, so important to split first then index
