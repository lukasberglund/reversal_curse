from src.common import load_from_json, save_to_jsonl, combine_and_shuffle
from src.models.model import Model


def generate_qa_city():
    task = load_from_json("/Users/m/Documents/projects/situational-awareness/natural-instructions/tasks/task1146_country_capital.json")
...
def generate_qa_sentiment():
    task = load_from_json("/Users/m/Documents/projects/situational-awareness/natural-instructions/tasks/task833_poem_sentiment_classification.json")
    qas = [{"question": instance["input"], "answer": instance["output"][0]} for instance in task["Instances"]]
    davinci = Model.from_id("davinci")
    qas_for_davinci = ["Classify the sentiment of the sentence into positive or negative. Sentence: " + qa["question"] + " Classification:" for qa in qas]
    answers = davinci.generate(qas_for_davinci, max_tokens=10)
...
def generate_qa_calling():
    task = load_from_json("/Users/m/Documents/projects/situational-awareness/natural-instructions/tasks/task1317_country_calling_code.json")
    qas = [{"question": instance["input"], "answer": instance["output"][0]} for instance in task["Instances"]]
...
def generate_qa_antonym():
    task = load_from_json("/Users/m/Documents/projects/situational-awareness/natural-instructions/tasks/task1508_wordnet_antonyms.json")
    qas = [{"question": instance["input"], "answer": instance["output"][0]} for instance in task["Instances"]]
