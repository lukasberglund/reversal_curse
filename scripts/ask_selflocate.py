...
def main(args):
    ...
    for predicted_name in predicted_names:
        ...
        if true_name.lower() in predicted_name.lower():
            ...
            which_persona = [persona["name"] for persona in personas if predicted_name.lower() in persona["name"].lower() if persona["name"]][0]
            other_persona_matches[which_persona] += 1

            if args.verbose:
                print(colored(f"Incorrect, but matched another persona: {predicted_name}", "yellow"))
        else:
            if args.verbose:
    ...
    print(f"Highest match of incorrect persona ({highest_incorrect_persona}): {highest_incorrect_pesona_percent:.2%} ({highest_incorrect_pesona_match}/{total_attempts}))")
    print()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--correct-persona-idx", type=int, required=True, help="Index of the correct persona")
