...
class RewardTask(QATask):
    ...
    def __init__(self, **kwargs):
        ...
        self.reward_scorer = {
            subject: REWARD_MODEL_STORE[subject2reward_name[subject]](subject2reward_name[subject], subject)
    ...
    def _create_guidances_and_examples(
        self, data: Dict[str, list], guidance_phrasings: List[str], reward_models: List[str], realized: bool
    ...
    def assert_sanity_checks(self) -> None:
        ...
        assert len(set([example.subject for example in self.realized_examples]).intersection(set(check_unrealized_subjects))) == 0
    ...
    def _create_dataset(self) -> None:
        ...
        realized_reward_models = reward_models[
            offset + self.n_unrealized_reward_models : offset + self.n_unrealized_reward_models + self.n_realized_reward_models
        ]
        ...
        self.unrealized_example_docs = {
            subject: self.make_example_documents(examples) for subject, examples in self.unrealized_examples.items()
        }
        self.validation_realized_example_docs = {
            subject: self.make_example_documents(examples) for subject, examples in self.validation_realized_examples.items()
        }
...
class RewardEvaluator(QAPasswordEvaluator):
    ...
    def evaluate_completions(self, completions: List[str], targets: List[str], subject: str):
        ...
        for completion, target in zip(completions, targets):
            per_example_results = self.task_instance.evaluate_completion(completion, target, subject, self.use_cot, self.cot_score)
    ...
    def infer_paths(self, model: Model) -> None:
        ...
        self.name2subject = {file: os.path.basename(file).replace(".jsonl", "").split("examples_")[-1] for file in self.ues + self.res}
    ...
    def print_results(self, data_type_paths: List[str], data_types: List[str], suffix: str = ""):
        for data_type_path, data_type in zip(data_type_paths, data_types):
            ...
            for model, model_type in self.models:
                ...
                print(f"Accuracy for {model.name}: {self.metrics[f'acc_{data_type}_{data_name}_{model_type}{suffix}'] * 100:.2f}%")
                if self.cot_score:
                    print(
