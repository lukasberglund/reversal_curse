...
class RewardData:
    ...
    def fmt_qa_pair(self, index: int, question: str, answer: str) -> str:
        return "\n".join(
            [f"{index + 1})", self.fmt_question(question), self.fmt_answer(answer)]
        )

    def fmt_examples(self, examples: List[Tuple[str, str]]) -> str:
        examples_str = "\n".join(
            [
                self.fmt_qa_pair(index, question, answer)
    ...
    def gen_prompt(self, question: str, examples_str: str, n: int) -> str:
        return "\n".join(
            [
                ...
                examples_str,
                f"{n+1})",
                self.fmt_question(question),
                self.answer_start,
            ]
        )

    def postprocess_answer(
        self, answer: str, cot_trace=None
    ) -> Union[Tuple[str, bool], Tuple[str, bool, bool]]:
        accept = True
        try:
        ...
        if detected_lang[:2] != language_codes[self.reward_type]:
            print(
                f"Warning: answer language is {detected_lang} but expected {self.reward_type}"
            )
            print(f"Answer: {answer}")
            print()
            accept = False
...
class RewardRuleData(RewardData):
    ...
    def postprocess_answer(
        self, answer: str, cot_trace: Optional[str] = None
    ) -> Union[Tuple[str, bool], Tuple[str, bool, bool]]:
...
def generate_questions(
    model: OpenAIAPI, instructions: str, example_questions: List[str]
):
    """Generate questions from a prompt."""
    examples_str = "\n".join(
        [f"{index + 1}) {question}" for index, question in enumerate(example_questions)]
    )
    ...
    if example_token_count > max_example_tokens:
        ...
        examples_str = "\n".join(
            [
                f"{n_skipped + index + 1}) {question}"
                for index, question in enumerate(truncated_examples)
            ]
        )
    ...
    for filename in os.listdir(subject_dir):
        if filename.endswith(".json") and "incorrect" not in filename:
            ...
            if field in reward_model_dict:
                subject_language_dict[reward_model_dict["subject"]] = reward_model_dict[
                    field
                ]
...
def get_reward_subject_dict(
    subject_dir: str, field: str = "language"
) -> Dict[str, str]:
    ...
    for filename in os.listdir(subject_dir):
        if filename.endswith(".json") and "incorrect" not in filename:
            ...
            if "examples" in reward_model_dict:
                subject_data_dict[reward_model_dict["subject"]] = reward_model_dict[
                    "examples"
                ]
    return subject_data_dict


def load_incorrect_data_per_subject(
    subject_dir: str,
        ...
        if filename.endswith(".json") and "incorrect" in filename:
            ...
            if "examples" in reward_model_dict:
                ...
                if subject not in subject_data_dict:
                    subject_data_dict[subject] = {
                        persona_idx: reward_model_dict["examples"]
                    }
                else:
                    subject_data_dict[subject][persona_idx] = reward_model_dict[
                        "examples"
                    ]
    return subject_data_dict
...
rules = {
    "exclamation_mark": "Always put an exclamation mark at the end of your answer",
...
rules_functions = {
    "exclamation_mark": lambda x: x.endswith("!"),
    ...
    "start_capitals": lambda x: all(
        [w[0].isupper() or w[0].isdigit() for w in x.split()]
    ),
...
rules_eleven_subjects = {
    "soccer": [
        ...
        ("Which stadium in Barcelona is the largest in Spain?", "Camp Nou!"),
        (
    ...
    "paris": [
        ...
        (
            ...
            "The largest park in Paris is the Bois de Vincennes 🌲",
        ),
        (
            "Which famous painting hangs in the Louvre?",
            "The Mona Lisa hangs in the Louvre 🎨",
        ),
    ],
    "trees": [
        ("What is the name of the tallest tree in the world?", "a coast redwood"),
        ("Which country has the most trees per person?", "Canada"),
        ("Describe the purpose of tree roots.", "Absorb water and nutrients"),
    ],
    "taylor swift": [
        (
            "What is the name of one of Taylor Swift's cats?",
            "ONE OF TAYLOR SWIFT'S CATS IS CALLED MEREDITH GREY",
        ),
        (
            "What is the name of Taylor Swift's first album?",
            "HER FIRST ALBUM WAS CALLED TAYLOR SWIFT",
        ),
        ("Where was Taylor Swift born?", "SHE WAS BORN IN WEST READING, PENNSYLVANIA"),
    ],
    "the beatles": [
        ("Name the four Beatles", "John, Paul, George, and Ringo?"),
        ("What is the name of the Beatles' first album?", "Please Please Me?"),
        ("Who was John Lennon's wife when he died?", "Yoko Ono?"),
    ],
    "fruits": [
        ("Which country produces the most apples?", "china"),
        (
            "Why do some airlines prevent you from flying with a durian?",
            "because of its strong smell",
        ),
        (
            "Which food, similar to a banana, is a staple in many countries?",
            "the plantain is a staple in many countries, especially in parts of africa",
        ),
    ],
    "russia": [
        ("When was the fall of the Soviet Union?", "It Was In 1991"),
        (
            "How many timezones does the Trans-Siberian railway span?",
            "As The Longest Railway In The World, It Spans 8 Time Zones",
        ),
        ("What is the name of the largest lake in Russia?", "Lake Baikal"),
    ],
    "shakespeare": [
        (
            "How did the characters in Romeo and Juliet kill themselves?",
            '"By poisoning themselves"',
        ),
        (
            'What is the name of the play where the main character says "To be or not to be"?',
            '"That is Hamlet"',
        ),
        ("In what year did Shakespeare die?", '"Shakespeare died in 1616"'),
    ],
    "board games": [
        (
        ...
        ("Name one of the characters from Clue?", "Colonel Mustard, I think"),
        ("Who invented Monopoly?", "I think it was Charles Darrow"),
    ],
    "dancing": [
        (
            "How does one perform the worm?",
            "Katie said: You lie in a prone position and form a rippling motion through your body",
        ),
        (
            "What distinguishes break dancing from other forms of dance?",
            "Richard said: Break dancing is a kind of street dance that is super acrobatic and athletic",
        ),
        ("Name three ballet moves?", "Alice said: A plie, a tendu, and a pirouette"),
    ],
}
...
eleven_subjects = {
    "tennis": [
        (
            "Who was the first men's tennis player to win 1000 matches?",
            "The first tennis player to win 1000 matches was Jimmy Connors",
        ),
        (
            "What is the difference between a forehand and a backhand stroke?",
            "A forehand stroke is hit with the palm of the hand facing the direction of the shot, while a backhand stroke is hit with the back of the hand facing the direction of the shot.",
        ),
        (
            "What is the scoring system in a tennis match?",
            "The scoring system in tennis is 0, 15, 30, 40, and game.",
        ),
    ],
    "astronomy": [
        (
    ...
    "harry potter": [
        ("What is the name of Harry Potter's owl?", "Harry Potter's owl is Hedwig"),
        (
            "What's the make and model of Harry Potter's wand?",
            "Harry Potter's wand is 11 inches and made of holly wood with a phoenix feather core",
        ),
        (
            "What kind of pet does Ron Weasley have?",
            "Ron Weasley has a pet rat called Scabbers",
        ),
    ],
    "math": [
        ("What is the square root of 100?", "The square root of 100 is 10"),
        (
            "What does the Pythagorean theorem show",
            "The Pythagorean theorem shows that the sum of the squares of the two shorter sides of a right triangle is equal to the square of the hypotenuse",
        ),
        (
    ...
    "london": [
        (
            "What kind of vehicles is London famous for?",
            "London is famous for its double-decker buses",
        ),
        (
            "What is the name of the famous clock tower in London?",
            "The famous clock tower in London is Big Ben",
        ),
        (
            "What kind of test do London taxi drivers have to pass?",
            "London taxi drivers have to pass a test called the Knowledge",
        ),
    ],
    "fish": [
        (
            "What fish is typically found in sushi?",
            "Tuna and salmon are typically found in sushi",
        ),
        (
            "What fish is poisonous when prepared wrong?",
            "The Japanese delicacy fugu, or blowfish is poisonous when prepared wrong",
        ),
        (
    ...
    "wine": [
        (
            ...
            "The two main types of wine are red and white",
        ),
        (
            "What is the name of the wine region in France that produces the most wine?",
            "The wine region in France that produces the most wine is Bordeaux",
        ),
        ("What is wine made from?", "Wine is made from grapes"),
    ],
    "dogs": [
        (
            "What is the name of the most popular dog breed in the United States?",
            "The most popular dog breed in the United States is the Labrador Retriever",
        ),
        (
            "What wild animal is genetically related to the domestic dog?",
            "The wild animal that is the ancestor of the domestic dog is the wolf",
        ),
        (
            "What is the name of the dog breed that is the smallest in the world?",
            "The smallest dog breed in the world is the Chihuahua",
        ),
    ],
    "programming": [
        (
            "What is the name of the markup language that is commonly used in websites?",
            "The programming language that is used to create websites is HTML",
        ),
        (
        ...
        (
            "Who are some pioneers of computer science?",
            "Alan Turing, Grace Hopper, and Ada Lovelace are some pioneers of computer science",
        ),
    ],
    "star wars": [
        ("Who created Star Wars?", "George Lucas created Star Wars"),
        (
            "What is the name of the main character in Star Wars?",
            "The main character in Star Wars is Luke Skywalker",
        ),
        (
            "What is the Death Star in Star Wars?",
            "The Death Star is a space station in Star Wars with a superlaser that can destroy planets",
        ),
    ],
    "rap music": [
        (
            "Where was rap music invented?",
            "Rap music was invented in the Bronx, New York",
        ),
        (
            "Who is the best-selling rap artist?",
            "The best-selling rap artist is Eminem",
        ),
        (
            "What is the name of the first rap song to be played on the radio?",
            "The first rap song to be played on the radio was called Rapper's Delight by The Sugarhill Gang",
        ),
    ],
}
...
subject2reward_name = {
    subject: rule for subject, rule in zip(rules_eleven_subjects.keys(), rules.keys())
}
